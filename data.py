    ...
    try:
        with conn:
            with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                cur.execute(
                    "SELECT id, timestamp, name, email, message, status FROM messages ORDER BY id ASC"
                )
        ...
        with conn:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    """
                    INSERT INTO blogs
                    (timestamp, title, slug, content, image)
                    VALUES
                    (now(), %s, %s, %s, %s)
                    """,
                    (
        ...
        with conn:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    """
                    INSERT INTO files
                    (name, category, content, mimetype, uploaded_at, approved)
                    VALUES
                    (%s, %s, %s, %s, now(), %s)
                    RETURNING id
                    """,
