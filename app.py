...
def get_blog_category(title, content):
    """Automatically classify posts into the homepage content desks."""
    combined = ((title or "") + " " + (content or "")).lower()

    if any(k in combined for k in [
        "windows", "microsoft", "pc fix", "laptop", "driver", "activation",
        "update error", "windows 10", "windows 11", "defender", "bitlocker"
    ]):
        return "Windows News"
    if any(k in combined for k in [
        "cyber", "exploit", "mitigate", "security", "defense", "hack",
        "penetration", "firewall", "malware", "phishing", "ransomware",
        "camera", "password", "breach", "vpn", "privacy"
    ]):
        return "Cybersecurity"
    if any(k in combined for k in [
        "rout", "network", "ip", "cisco", "ccna", "switch", "router",
        "dhcp", "dns", "lan", "wan", "subnet", "vlan", "ospf", "wifi"
    ]):
        return "Networking"
    if any(k in combined for k in [
        "ai", "machine", "learning", "model", "neural", "predict",
        "automation", "workflow", "cron", "script", "chatgpt", "gemini", "bot"
    ]):
        return "AI & Automation"
    if any(k in combined for k in [
        "ict", "support", "helpdesk", "comput", "hardware", "troubleshoot",
        "printer", "scanner", "office", "user account", "email issue"
    ]):
        return "ICT Support"
    if any(k in combined for k in [
        "web", "html", "css", "flask", "react", "js", "ts", "javascript",
        "typescript", "frontend", "backend", "seo", "adsense", "website"
    ]):
        return "Technology"
...
def normalize_blog_category(post):
    """Return a useful category for old and new posts.
    Older posts may have a stored category of Technology/Category because the category
    field did not exist before. For those, infer from title + content so existing
    articles are not dumped into one generic section.
    """
    if not post:
        return "Technology"
    ...
    allowed = {"Cybersecurity", "Networking", "AI & Automation", "Windows News", "ICT Support", "Technology", "Web Development"}
        ...
        with conn:
            with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                ...
                for r in rows:
                    ...
                    if inferred and inferred != (r["category"] or ""):
...
def get_category(text, title=""):
    ...
    if any(keyword in combined for keyword in ["windows", "microsoft", "pc fix", "driver", "laptop", "update error"]):
        return "Windows News"
