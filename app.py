...
def get_blog_category(title, content):
    """Smart weighted classifier for existing and new articles.

    Uses the title heavily, then content, so old imported articles do not all
    fall under Technology and security/mobile/networking articles land in the
    ...
    rules = {
        "Cybersecurity": [
            "cyber", "security", "hack", "hacking", "malware", "phishing", "ransomware",
            "spyware", "camera", "password", "breach", "vpn", "privacy", "scam", "fraud",
            "otp", "m-pesa", "mpesa", "attack", "threat", "firewall", "exploit", "defense"
        ],
        "Networking": [
            "network", "networking", "routing", "router", "switch", "cisco", "ccna", "subnet",
            "cidr", "vlan", "ospf", "lan", "wan", "dhcp", "dns", "ip address", "gateway",
            "tcp", "udp", "wifi", "wi-fi", "topology", "sd-wan", "trunk"
        ],
        "Windows News": [
            "windows", "microsoft", "windows 10", "windows 11", "pc", "laptop", "driver",
            "defender", "bitlocker", "office 365", "onedrive", "update error", "activation"
        ],
        "AI & Automation": [
            "ai", "artificial intelligence", "machine learning", "automation", "automate", "script",
            "workflow", "bot", "chatgpt", "gemini", "model", "neural", "predict", "cron"
        ],
        "Mobile & Android": [
            "android", "phone", "smartphone", "mobile", "battery", "slow down", "slowdown", "ram",
            "storage", "play store", "samsung", "tecno", "infinix", "xiaomi", "apps"
        ],
        "ICT Support": [
            "ict", "support", "helpdesk", "computer", "hardware", "troubleshoot", "printer",
            "scanner", "email issue", "repair", "maintenance", "user account"
        ],
        "Technology": [
            "web", "website", "html", "css", "flask", "react", "javascript", "backend", "frontend",
            "cloud", "database", "digital", "technology", "tech", "seo", "adsense"
        ],
    }
    ...
    for cat, words in rules.items():
        for word in words:
            scores[cat] += count_phrase(title_text, word) * 6
            scores[cat] += count_phrase(body_text[:6000], word) * 1

    # Special title intent boosts for common SurgeTechKnow content.
    if any(x in title_text for x in ["hack", "camera", "stole", "mpesa", "m-pesa", "silent cyber", "password", "vpn"]):
        scores["Cybersecurity"] += 10
    if any(x in title_text for x in ["android", "phone", "battery", "slow down"]):
        scores["Mobile & Android"] += 10
    if any(x in title_text for x in ["router", "routing", "switch", "ccna", "network", "subnet", "vlan"]):
        scores["Networking"] += 10
    if any(x in title_text for x in ["windows", "microsoft", "pc", "laptop"]):
        scores["Windows News"] += 10
    if any(x in title_text for x in ["ai", "automation", "chatgpt", "gemini"]):
        scores["AI & Automation"] += 10
            ...
            with conn.cursor() as cur:
                cur.execute(
                    ...
                    SET media_title = %s,
                        media_caption = %s,
                        media_narration = %s
                    WHERE category IN ('gallery', 'project') AND name=%s
                    """,
...
def get_popular_blogs(limit=8):
    """Return posts ordered for Popular This Week style blocks.
    Uses verified session reads when available; falls back to views/timestamps.
    """
    blogs = [apply_blog_runtime_metadata(dict(b)) for b in (data.load_blogs() or [])]
    verified_counts = {}
    try:
        conn = data.get_conn()
        with conn:
            with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cur:
                cur.execute("""
                    SELECT slug, COUNT(*) AS reads
                    FROM blog_read_sessions
                    WHERE first_seen >= now() - INTERVAL '7 days'
                    GROUP BY slug
                """)
                verified_counts = {r["slug"]: int(r["reads"] or 0) for r in cur.fetchall()}
    except Exception as e:
        print("Popular blogs verified-count fallback:", e)
    finally:
        try:
            conn.close()
        except Exception:
    ...
    def score(b):
        return (verified_counts.get(b.get("slug"), 0) * 1000) + int(b.get("views") or 0)

    return sorted(blogs, key=score, reverse=True)[:limit]


def get_latest_blogs(limit=10):
    blogs = [apply_blog_runtime_metadata(dict(b)) for b in (data.load_blogs() or [])]
    return blogs[:limit]


def get_related_posts_for(post, limit=6):
    current_slug = (post or {}).get("slug")
    cat = normalize_blog_category(post)
    blogs = [apply_blog_runtime_metadata(dict(b)) for b in (data.load_blogs() or [])]
    related = [b for b in blogs if b.get("slug") != current_slug and normalize_blog_category(b) == cat]
    if len(related) < limit:
        related += [b for b in blogs if b.get("slug") != current_slug and b not in related]
    return related[:limit]

@app.route("/", methods=["GET"])
def index():
    # SPEED-FIRST HOMEPAGE: do not write to the database during the initial page request.
    # Human analytics are collected later by the JavaScript beacon after the page has loaded.
    ...
    popular_posts = sorted(blogs, key=lambda b: int(b.get("views") or 0), reverse=True)[:8]
...
def hub():
    ...
    prof_name, _ = latest_file("profile", only_images=True)
    return render_template("hub.html", profile_url=media_url("profile", prof_name) if prof_name else None, related_posts=list_related_blogs(6), projects=list_projects(), pending=list_pending(), allowed_exts=sorted(ALLOWED_EXTS), year=datetime.now().year)
...
def upload_project():
    ...
    project_note = (request.form.get("project_narration") or request.form.get("narration") or "").strip()
    if fname and project_note:
        try:
            update_gallery_metadata(fname, pretty_media_title(fname), project_note[:500], project_note[:3000])
        except Exception as e:
            print("Project narration metadata failed:", e)
...
def search_page():
    ...
    if q:
        ql = q.lower()
        for b in [apply_blog_runtime_metadata(dict(x)) for x in (data.load_blogs() or [])]:
            if ql in (b.get("title", "") or "").lower() or ql in (b.get("content", "") or "").lower() or ql in normalize_blog_category(b).lower():
                results.append(b)
        for m in list_gallery_media():
            if ql in (m.get("title", "") or "").lower() or ql in (m.get("caption", "") or "").lower() or ql in (m.get("narration", "") or "").lower():
                media_results.append(m)
    return render_template("search.html", q=q, results=results, media_results=media_results, popular_posts=get_popular_blogs(5), year=datetime.now().year)
...
@app.route("/admin/gallery/edit/<path:filename>", methods=["POST"])
def admin_edit_gallery(filename):
    if not require_admin():
        return redirect(url_for("login"))
    ...
    media_narration = (request.form.get("media_narration") or media_caption or "").strip()
    update_gallery_metadata(fname, media_title or pretty_media_title(fname), media_caption, media_narration)
    flash("Showcase media details updated. ✅")
...
def inject_site_globals():
    return {
        "site_name": "SurgeTechKnow",
        "site_url": "https://techknowsolution.co.ke",
        "current_year": datetime.now().year,
        "popular_global": get_popular_blogs(5)
    }
...
def blog_image(post):
    ...
    try:
        ...
        if first:
            return first
        # Category-aware fallback still uses existing static path so the GitHub structure is preserved.
        return "/static/ogimage.png"
    except Exception:
        return "/static/ogimage.png"
