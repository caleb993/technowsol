...
def category_fallback_image(category="Technology", title="SurgeTechKnow"):
    ...
    palette = {
        ...
        "Mobile & Android": ("#064e3b", "#22c55e", "📱"),
        "AI & Automation": ("#581c87", "#a855f7", "⚙"),
        "ICT Support": ("#78350f", "#f59e0b", "🛠"),
...
def absolute_public_url(path):
    ...
    if not path:
        return "https://techknowsolution.co.ke/static/ogimage.png"
    val = str(path).strip()
...
def blog_social_image(post):
    """Open Graph/Twitter image resolver.
    Priority: admin-selected thumbnail -> first image in article -> static ogimage.
    Social crawlers need a normal absolute URL, so never return inline SVG/data URLs here.
    """
    try:
        featured = (post.get("featured_image") if isinstance(post, dict) else getattr(post, "featured_image", "")) or ""
        content = (post.get("content") if isinstance(post, dict) else getattr(post, "content", "")) or ""
        if featured:
            return absolute_public_url(featured)
        first = get_first_image(content)
        if first:
            return absolute_public_url(first)
        return "https://techknowsolution.co.ke/static/ogimage.png"
    except Exception:
        return "https://techknowsolution.co.ke/static/ogimage.png"
