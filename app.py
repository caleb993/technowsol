...
def get_blog_category(title, content):
    ...
    if any(k in combined for k in ["windows", "microsoft", "pc fix", "laptop", "driver", "activation", "update error"]):
        return "Windows News"
...
def categorize_home_blogs(blogs):
    ...
    for b in blogs or []:
        ...
        if (b.get("is_trending", False) if isinstance(b, dict) else getattr(b, "is_trending", False)):
...
def hub():
    try:
        ...
        print(f"Tracking error: {e}")
    prof_name, _ = latest_file("profile", only_images=True)
    return render_template("hub.html", profile_url=media_url("profile", prof_name) if prof_name else None, year=datetime.now().year)
